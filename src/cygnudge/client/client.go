...
func logout(add ServerAddress,uid int,token string){
	...
	1. send uid
	2. send token
	3. server: if rdb.Get(uid+"-token")==token: remove token in rdb
	else: return response "Not Acceptable"
	*/
	conn,err:=net.Dial("tcp",add.Ip+":"+strconv.Itoa(add.Port))
	if err!=nil{
		log.Fatalln(err)
...
func main(){
	...
	case "register":{
		...
		if registerIp==""{//use server name
			...
			server_json_string,err:=ioutil.ReadFile(server_json_path)
			if err!=nil{
				log.Fatalln(err)
			}
			...
			if !exist{
				fmt.Println("cygnudge register: server %s not found",registerServer)
				os.Exit(2)
			}//not found
			server_address=tmp
		}else{
	...
	case "login":{
		...
		if loginIp==""{//use server name
			...
			if !exist{
				fmt.Println("cygnudge login: server %s not found",loginServer)
				os.Exit(3)
			}//not found
			server_address=tmp
		}else{
		...
		if os.IsNotExist(err){//token doesn't exist
			...
			login_token_tmp,err:=ioutil.ReadFile(login_token_path)
	...
	case "logout":{
		...
		if logoutIp==""{//use server name
			...
			if !exist{
				fmt.Println("cygnudge logout: server %s not found",logoutServer)
				os.Exit(4)
			}//not found
			server_address=tmp
		}else{
		...
		var login_token_string string
		if os.IsNotExist(err){//token doesn't exist
			fmt.Printf("cygnudge logout: no user login")
			os.Exit(4)
		}else{
			login_token_tmp,err:=ioutil.ReadFile(login_token_path)
			login_token_string=string(login_token_tmp)
		...
		if os.IsNotExist(err){
			...
			login_uid_tmp,err:=ioutil.ReadFile(login_uid_path)
		...
		case "OK":
			...
			removeLoginUidToken()
		case "Not Acceptable":
			fmt.Printf("cygnudge logout: invalid local token")
